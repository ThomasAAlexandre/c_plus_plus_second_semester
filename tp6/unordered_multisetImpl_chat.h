...
unordered_multiset<TYPE, classe_de_dispersion>::insert(const TYPE& val)
{
    ...
    if (m_rep[alv] == nullptr) {
        m_rep[alv] = new std::list<TYPE>(); // Create a new bucket if it doesn't exist
    }

    // Check if the element already exists in the bucket
    if (std::find(m_rep[alv]->begin(), m_rep[alv]->end(), val) == m_rep[alv]->end()) {
        // Element not found, add it to the bucket
        m_rep[alv]->push_back(val);
        m_size++; // Increment the size of the unordered_multiset
    }

    if (static_cast<double>(m_size) / m_rep.size() > m_facteur_max) {
        rehash(m_rep.size() * 2); // Double the number of buckets if load factor exceeds m_facteur_max
    }
